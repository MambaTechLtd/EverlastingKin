...
import { Users, FileText, Shield, Clock, CheckCircle, XCircle, AlertTriangle, Eye, TrendingUp, Activity } from 'lucide-react'
    ...
    <div className="bg-ek-bg-main border border-ek-accent-gold/30 rounded-xl p-6 hover:border-ek-accent-mint/50 transition-all duration-300 group">
      <div className="flex items-center justify-between">
        <div className="flex-1">
          ...
          {trend && (
            <div className={`flex items-center space-x-1 mt-2 text-xs ${trend.isPositive ? 'text-ek-success' : 'text-ek-error'}`}>
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-3xl font-bold text-ek-text-main">System Administration</h1>
          <p className="text-ek-text-muted mt-2">
            Comprehensive oversight and management of the Everlasting Kin platform
          </p>
        </div>
        <div className="mt-4 sm:mt-0 flex items-center space-x-2">
          <div className="flex items-center space-x-2 text-sm text-ek-text-muted">
            <Activity className="w-4 h-4 text-ek-success" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-ek-bg-main border border-ek-accent-gold/30 rounded-xl p-6">
          <h3 className="text-lg font-semibold text-ek-text-main mb-4">User Distribution</h3>
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              <span className="text-ek-text-muted">Administrators</span>
              <span className="font-medium text-ek-text-main">{stats.user_counts?.admins || 0}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-ek-text-muted">Mortuary Staff</span>
              <span className="font-medium text-ek-text-main">{stats.user_counts?.mortuary_staff || 0}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-ek-text-muted">Police Officers</span>
              <span className="font-medium text-ek-text-main">{stats.user_counts?.police || 0}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-ek-text-muted">Public Users</span>
              <span className="font-medium text-ek-text-main">{stats.user_counts?.public_users || 0}</span>
            </div>
          </div>
        </div>

        <div className="bg-ek-bg-main border border-ek-accent-gold/30 rounded-xl p-6">
          <h3 className="text-lg font-semibold text-ek-text-main mb-4">Record Status</h3>
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              <span className="text-ek-text-muted">Unidentified</span>
              <span className="font-medium text-ek-error">{stats.deceased_records?.unidentified || 0}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-ek-text-muted">Pending Confirmation</span>
              <span className="font-medium text-ek-accent-gold">{stats.deceased_records?.pending_confirmation || 0}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-ek-text-muted">Identified</span>
              <span className="font-medium text-ek-success">{stats.deceased_records?.identified || 0}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-ek-text-muted">Public Records</span>
              <span className="font-medium text-ek-accent-mint">{stats.deceased_records?.public || 0}</span>
            </div>
          </div>
        </div>

        <div className="bg-ek-bg-main border border-ek-accent-gold/30 rounded-xl p-6">
          <h3 className="text-lg font-semibold text-ek-text-main mb-4">Report Status</h3>
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              <span className="text-ek-text-muted">Draft Reports</span>
              <span className="font-medium text-ek-text-muted">{stats.police_reports?.draft || 0}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-ek-text-muted">Submitted</span>
              <span className="font-medium text-ek-accent-gold">{stats.police_reports?.submitted || 0}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-ek-text-muted">Closed</span>
              <span className="font-medium text-ek-success">{stats.police_reports?.closed || 0}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mt-8">
              ...
              <div className="bg-ek-bg-main border border-ek-accent-gold/30 rounded-xl">
                ...
                <div className="divide-y divide-ek-accent-gold/20">
                  {stats.pending_users.slice(0, 5).map((user: AppUser) => (
                    <div key={user.id} className="p-6 flex items-center justify-between hover:bg-ek-accent-gold/5 transition-colors">
                      <div className="flex-1">
                      ...
                      <div className="flex items-center space-x-3">
                        <button
                          ...
                          className="flex items-center space-x-1 bg-ek-success hover:bg-ek-success/80 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors"
                        >
                        ...
                        <button
                          ...
                          className="flex items-center space-x-1 bg-ek-error hover:bg-ek-error/80 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors"
                        >
        ...
        {activeTab !== 'overview' && (
          <div className="bg-ek-bg-main border border-ek-accent-gold/30 rounded-xl">
            <div className="p-6 border-b border-ek-accent-gold/20">
              <h3 className="text-lg font-semibold text-ek-text-main">
